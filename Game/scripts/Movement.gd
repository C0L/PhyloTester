...
func set_movement_dir():
	...
	cmd.right_move = 0.0
	cmd.forward_move += int(Input.is_action_pressed("movement_forward"))
	cmd.forward_move -= int(Input.is_action_pressed("movement_backward"))
	cmd.right_move += int(Input.is_action_pressed("movement_right"))
	cmd.right_move -= int(Input.is_action_pressed("movement_left"))

func queue_jump():
	if Input.is_action_just_pressed("movement_jump") and !wish_jump:
		wish_jump = true
...
func _input(event):
	if event is InputEventMouseMotion and Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		...
		PlayerView.rotate_x(deg2rad(-event.relative.y * MOUSE_SENSITIVITY))
